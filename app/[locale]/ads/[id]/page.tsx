...
export default async function AdDetailPage({
  ...
  const mainImage = mediaArray.length > 0
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-1/2 w-full flex flex-col gap-4 p-6">
            <div className="flex flex-wrap gap-2 mb-2">
              ...
              {ad.is_chat_enabled && <Badge variant="outline">Chat Enabled</Badge>}
              {ad.contact_info_enabled && <Badge variant="outline">Contact Info Enabled</Badge>}
            </div>
            <h2 className="text-2xl font-bold mb-2">{ad.category?.name || "Ad"}</h2>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                <span className="text-muted-foreground">Form:</span> <span className="font-semibold">{ad.form}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Grade:</span> <span className="font-semibold">{ad.grade}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Surface:</span> <span className="font-semibold">{ad.surface?.name || ad.surface}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Category:</span> <span className="font-semibold">{ad.category?.name}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Weight:</span> <span className="font-semibold">{ad.weight}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Sale Unit:</span> <span className="font-semibold">{ad.sale_unit_type}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Min Order:</span> <span className="font-semibold">{ad.minimum_order}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Status:</span> <span className="font-semibold">{ad.status}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Origin Country:</span> <span className="font-semibold">{ad.origin_country?.name}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Origin City:</span> <span className="font-semibold">{ad.origin_city?.name}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Size:</span> <span className="font-semibold">{ad.size ? `${ad.size.h ?? "-"} × ${ad.size.w ?? "-"} × ${ad.size.l ?? "-"}` : "-"}</span>
              </div>
            </div>
            {/* Ports */}
            <div className="mt-4">
